...
if __name__ == "__main__":
    ...
    parser.add_argument('--soil_parameter', type=str, default='Moisture_perc_10', help='soil feature targer', required=True)
    ...
    np.random.seed(42)
    for parameter in [str(args.soil_parameter)]:
        print(parameter)
        ...
        dict_for_statistic_ks  = {'MAXVOL':{},
                            'cLHS':{}, 
                            'Random':{}}
            
        dict_for_pvalue  = {'MAXVOL':{},
                            'cLHS':{}, 
                            'Random':{}}

        # number_of_points = range(3,51)
        number_of_points = [3,4,6]
        #Read indices of points for cLHS
        # res = !ls one_mi*
        for num_points in number_of_points:
            SAR = Divergence()
            SAR.soil_feature = field_parameter
            ...
            for iteration in range(100):
                ...
                cLHS_dist = SAR.voronoi_map[cLHS]
                list_for_random_plots.append(cLHS_dist)

                cLHS_statistic.append(ks_2samp(SAR.original_data, cLHS_dist)[0])
                cLHS_pValue.append(ks_2samp(SAR.original_data, cLHS_dist)[1])
                cLHS_wasserstein.append(wasserstein_distance(SAR.original_data, cLHS_dist))
            
            dict_for_statistic_ks['cLHS'][num_points] = cLHS_statistic
            dict_for_pvalue['cLHS'][num_points] = cLHS_pValue
            dict_for_wasserstein['cLHS'][num_points] = cLHS_wasserstein
        ...
        #MAXVOL 
            ...
            MAXVOL = SAR.voronoi_map[SAR.i_am_maxvol_function()]
            
            dict_for_wasserstein['MAXVOL'][num_points] = wasserstein_distance(SAR.original_data, MAXVOL)
            dict_for_statistic_ks['MAXVOL'][num_points]  = ks_2samp(SAR.original_data, MAXVOL)[0]
            dict_for_pvalue['MAXVOL'][num_points]  = ks_2samp(SAR.original_data, MAXVOL)[1]
            # print('stat',ks_2samp(SAR.original_data, MAXVOL)[0])
            ...
            for iteration in range(100):
                ...
                RANDOM = SAR.voronoi_map[SAR.i_am_random()]
                random_statistic.append(ks_2samp(SAR.original_data, RANDOM)[0])
                random_pvalue.append(ks_2samp(SAR.original_data, RANDOM)[1])
                random_wasserstein.append(wasserstein_distance(SAR.original_data, RANDOM))

            dict_for_statistic_ks['Random'][num_points] = random_statistic
            dict_for_pvalue['Random'][num_points] = random_pvalue
            dict_for_wasserstein['Random'][num_points] = random_wasserstein


        np.save('./npy_files/'+parameter+'_wasserstein.npy', dict_for_wasserstein)
        np.save('./npy_files/'+parameter+'_ks_distance.npy', dict_for_statistic_ks)
        np.save('./npy_files/'+parameter+'_pvalue.npy', dict_for_pvalue)
        print('Finished!')